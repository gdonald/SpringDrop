...
/**
 * A value chosen from a list of text options. The values an editor may pick from live in the storage's
 * {@code allowed_values} setting, keyed by the stored value, so changing a label
 * leaves stored values untouched.
 */
@SpringDropPlugin(id = ListStringFieldType.ID, type = FieldType.class)
public class ListStringFieldType implements FieldType {

    public static final String ID = "list_string";
